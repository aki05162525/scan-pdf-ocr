...
const STATUS_MAP: Record<string, { label: string; color: string }> = {
  ...
  ocr_running: { label: "OCR処理中...", color: "bg-yellow-100 text-yellow-700" },
...
export default function JobStatus({ job }: Props) {
  ...
  const isProcessing = ["uploaded", "pdf_generated", "ocr_running"].includes(job.status);
    ...
    <div className="bg-white rounded-lg border p-6">
      ...
      <dl className="grid grid-cols-2 gap-4 text-sm">
        ...
        <div>
          ...
          <dd className="font-medium">{job.language === "jpn" ? "日本語" : "日本語 + 英語"}</dd>
        </div>
        <div>
          <dt className="text-gray-500">開始日時</dt>
          <dd className="font-medium">{new Date(job.createdAt).toLocaleString("ja-JP")}</dd>
        </div>
      </dl>
